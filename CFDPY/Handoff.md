...
### 1.6 Tecplot `.dat` format modernized (py2tec dialect)
The original `TecplotExporter.write` emitted the legacy `F=POINT` token together
with `DATAPACKING=POINT` — a deprecated finite-element specifier that conflicts
with the data-packing directive and is rejected by current Tecplot 360 (the
files would not load).  Rewritten to the modern dialect used by the
[py2tec](https://github.com/luohancfd/py2tec) tools:
...
Changes: comma-separated `VARIABLES`; `ZONETYPE=ORDERED` replaces `F=POINT`;
added zone title `T=`, `STRANDID=1`, `SOLUTIONTIME`; kept POINT data packing
(one record per node) and the Fortran-order node flattening (I fastest), which
is unchanged and correct.  Validated by round-tripping a file through
`py2tec.tec2py` (all 9 variables, correct node count and grid layout).  All
197 existing `frame_*.dat` in `outputs/liquid_drop_splash_2D/` were
regenerated from the saved HDF5 snapshots in the new format.
...
| `visualization/tecplot_writer.py` | **Tecplot `.dat` rewritten to the modern `ZONETYPE=ORDERED`/`DATAPACKING=POINT` dialect (py2tec-compatible); dropped the legacy `F=POINT` token that broke current Tecplot 360.** Round-trips through `py2tec.tec2py`.** |
