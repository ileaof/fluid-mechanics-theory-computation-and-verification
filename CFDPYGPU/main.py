...
from solver import (BoundaryCondition, LinearSolver, ProjectionMethod,
                    EnergySolver, VOFSolver, ForcesCalculator, IBMForcing,
                    CutCellGeometry)
from visualization import MatplotlibViewer, TecplotExporter, PostProcessor

# GPU acceleration (lazy, side-effect free): only the hardware report is
# imported eagerly so it can be printed at startup.  The backend / kernels are
# imported lazily by the solver modules only when a GPU is actually in use, so a
# CPU-only machine pays no GPU import cost.
try:
    from gpu import print_hardware_report, gpu_available
    _GPU_IMPORT_OK = True
except Exception:                       # pragma: no cover - gpu pkg optional
    _GPU_IMPORT_OK = False

    def gpu_available() -> bool:
        return False

    def print_hardware_report(*_a, **_k) -> None:  # type: ignore[no-redef]
        pass
...
class Simulation:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        # obstacle dicts in ``cfg.obstacles`` (boxes, cylinders, spheres); a
        # cell is solid when its centre lies inside any obstacle.  The mask is
        # registered on the boundary condition so every face-flux / gradient
        # routine zeroes the solid faces (direct-forcing no-flux on the
        # collocated grid).  When ``immersed_method == "ibm"`` a curved-body
        # ghost-cell forcing is also registered so the no-slip is placed at the
        # true surface instead of the staircase cell face.
        solid = self._build_solid_mask()
        if solid is not None:
            self.bc.set_solid(solid)
        self.ibm: IBMForcing | None = None
        self.cut_cell = None
        # Cut-cell pressure-Poisson for the curved-body IBM.  When enabled,
        # the centre-test solid mask is replaced by the volume-fraction
        # ``is_solid`` (vf == 0) so centre-inside *cut* cells (vf > 0) are
        # treated as fluid, and the no-penetration wall in the Poisson
        # divergence is placed at the true surface via the cut-cell geometry.
        # The ghost-cell IBM is rebuilt on this vf-based mask so its ghosts are
        # the deep-solid ring (vf == 0), not the now-fluid cut cells.
        ibm_mask = solid
        if (solid is not None and cfg.immersed_method == "ibm"
                and cfg.ibm_cut_cell and self.bc.has_solid):
            cc = CutCellGeometry(self.mesh, solid, list(cfg.obstacles))
            if cc.has_curve:
                self.cut_cell = cc
                ibm_mask = cc.is_solid
                self.bc.set_solid(cc.is_solid)
                self.projection.pressure.set_cut_cell(cc)
            # no curved body -> fall through with the centre mask (staircase)
        if (ibm_mask is not None and cfg.immersed_method == "ibm"
                and self.bc.has_solid):
            ibm = IBMForcing(self.mesh, ibm_mask, list(cfg.obstacles))
            if ibm.active:
                self.bc.set_immersion(ibm)
                self.ibm = ibm

        # Force integration on the immersed obstacle (drag / lift).  Built
        # only when requested and a solid mask exists; the free-stream velocity
        # is read from the west inlet BC and the reference length from the
        # obstacle geometry (cylinder diameter, else the mask extent).
        self.forces_calc: ForcesCalculator | None = None
        if cfg.compute_forces and self.bc.has_solid:
            U_inf, D = self._force_reference()
            self.forces_calc = ForcesCalculator(self.mesh, self.bc, self.fluid,
                                                 U_inf, D)
    ...
    def _build_solid_mask(self) -> np.ndarray | None:
        ...
        # Optionally snap curved-body centres to the nearest grid line (cell
        # centre or face) so the rasterised staircase is mirror-symmetric and
        # does not produce a spurious steady lift.  The snapped centre is
        # written back into the obstacle dict so the IBM geometry and the force
        # integration see the same body as the mask.
        if self.cfg.snap_obstacle_to_grid:
            self._snap_obstacle_centres()
        for ob in self.cfg.obstacles:
            ...
            if shape == "box":
                ...
                cx, cy = ob["center"]
                r = ob["radius"]
                # Cylinder of infinite extent along the configured axis: a
                # cell is inside when its centre is within r of the axis in
    ...
    def _snap_obstacle_centres(self) -> None:
        """Snap curved-obstacle centres to the nearest grid line, in place.

        For each cylinder / sphere obstacle the in-plane centre coordinates are
        rounded to the nearest multiple of ``h/2`` (a cell centre *or* a face),
        which makes the cell-centre ``inside`` test mirror-symmetric about that
        grid line.  The snap moves the body by at most ``h/4`` along each axis
        -- inside the O(h) rasterisation tolerance -- and is written back into
        the obstacle dict so :class:`IBMForcing` and :class:`ForcesCalculator`
        see the same centre as the mask.  Boxes are left untouched (axis-aligned
        boxes are already exact to O(0) when their bounds are on grid lines).
        """
        mesh = self.mesh
        h = (mesh.dx, mesh.dy, mesh.dz)

        def snap_axis(val: float, axis: int) -> float:
            hh = h[axis] * 0.5
            return float(round(val / hh) * hh)
        ...
        for ob in self.cfg.obstacles:
            ...
            if shape == "cylinder":
                cx, cy = ob["center"]
                axis = ob.get("axis", "z")
                # Snap the two coordinates perpendicular to the cylinder axis.
                if axis == "z":
                    ob["center"] = [snap_axis(cx, 0), snap_axis(cy, 1)]
                elif axis == "y":
                    ob["center"] = [snap_axis(cx, 0), snap_axis(cy, 2)]
                else:  # axis == "x"
                    ob["center"] = [snap_axis(cx, 1), snap_axis(cy, 2)]
            elif shape == "sphere":
                cx, cy, cz = ob["center"]
                ob["center"] = [snap_axis(cx, 0), snap_axis(cy, 1),
                                snap_axis(cz, 2)]
    ...
    def _apply_obstacle(self, u: np.ndarray, v: np.ndarray,
        ...
        """Enforce the immersed-boundary no-slip on the velocity field.

        Delegates to :meth:`BoundaryCondition.apply_immersion`: the
        mirror-point ghost-cell IBM when active, otherwise the staircase
        direct-forcing clamp to zero inside every solid cell.
        """
    ...
    def _force_reference(self) -> tuple[float, float]:
        """Free-stream velocity and reference length for the force coefficients.

        ``U_inf`` is read from the west inlet BC value (falling back to ``u0``
        or 1.0); ``D`` is the cylinder diameter when a cylinder obstacle is
        configured, otherwise the largest extent of the solid mask.
        """
        cfg = self.cfg
        west = self.bc.velocity.get("west")
        if west is not None and west.kind == "inlet" and abs(west.value) > 1e-30:
            U_inf = abs(west.value)
        else:
            U_inf = abs(cfg.u0) if abs(cfg.u0) > 1e-30 else 1.0
        D = 0.0
        for ob in cfg.obstacles:
            if ob.get("shape") == "cylinder":
                D = 2.0 * ob["radius"]
                break
            if ob.get("shape") == "sphere":
                D = 2.0 * ob["radius"]
                break
        if D <= 0.0 and self.bc.solid is not None:
            s = self.bc.solid
            Xc, Yc, _ = self.mesh.cell_grid()
            if s.any():
                D = max(float(Xc[s].max() - Xc[s].min()),
                        float(Yc[s].max() - Yc[s].min()))
        if D <= 0.0:
            D = 1.0
        return U_inf, D
    ...
    def step(self) -> dict[str, Any]:
        ...
        # Drag / lift coefficients on the immersed obstacle.
        if self.forces_calc is not None:
            fc = self.forces_calc.forces(s.u, s.v, s.p)
            diag["Cd"] = fc["Cd"]
            diag["Cl"] = fc["Cl"]
        return diag
    ...
    def _snapshot(self) -> dict[str, np.ndarray]:
        ...
        # Vorticity for the force-integration / cylinder cases (stored so the
        # viewer can animate it and HDF5 frames carry it for restarts).
        if self.forces_calc is not None:
            snap["vort"] = self.post.vorticity(s.u, s.v)
        return snap
    ...
    def _plot_force_history(self) -> None:
        """Render the Cd(t) / Cl(t) history and the Cl power spectrum (PNGs).

        Reads ``Cd`` / ``Cl`` from :attr:`history` (recorded every step when
        ``compute_forces`` is on).  The spectrum peak gives the vortex-shedding
        frequency and hence the Strouhal number ``St = f D / U_inf``.
        """
        import os
        import numpy as np
        rows = [h for h in self.history
                if "Cd" in h and "Cl" in h and isinstance(h.get("t"), (int, float))]
        if not rows:
            return
        try:
            import matplotlib
            ...
            return
        t = np.array([float(h["t"]) for h in rows])
        cd = np.array([float(h["Cd"]) for h in rows])
        cl = np.array([float(h["Cl"]) for h in rows])
        U_inf = self.forces_calc.U_inf
        D = self.forces_calc.D

        fig, ax = plt.subplots(2, 1, figsize=(8, 6), sharex=True)
        ax[0].plot(t, cd, color="C0", lw=1.0)
        ax[0].set_ylabel("$C_d$")
        ax[0].set_title(f"Drag coefficient (mean={cd.mean():.4f})")
        ax[1].plot(t, cl, color="C1", lw=1.0)
        ax[1].set_ylabel("$C_l$")
        ax[1].set_xlabel("t  [s]")
        ax[1].set_title(f"Lift coefficient (rms={cl.std():.4f})")
        fig.tight_layout()
        fig.savefig(os.path.join(self.cfg.output_dir, f"{self.cfg.name}_forces.png"),
                    dpi=120)
        plt.close(fig)

        # Cl power spectrum -> shedding frequency / Strouhal.  Use a
        # detrended, windowed FFT on the (assumed) uniform-ish sampled history.
        if t.size > 16 and U_inf > 1e-30 and D > 1e-30:
            ...
            if dt > 1e-12:
                ...
                # drop the DC bin
                if freqs.size > 1:
                    peak = 1 + int(np.argmax(spec[1:]))
                    f_peak = float(freqs[peak])
                    St = f_peak * D / U_inf
                else:
                    f_peak, St = 0.0, 0.0
                fig, ax = plt.subplots(figsize=(7, 4))
                ax.semilogy(freqs[1:], spec[1:], color="C2", lw=1.0)
                ax.set_xlabel("frequency  [Hz]")
                ax.set_ylabel("$|\\hat{C_l}|^2$")
                ax.set_title(f"Cl spectrum  peak f={f_peak:.4f} Hz  St={St:.4f}")
                fig.tight_layout()
                fig.savefig(os.path.join(self.cfg.output_dir,
                                         f"{self.cfg.name}_cl_fft.png"), dpi=120)
                plt.close(fig)
    ...
    def finalize(self) -> None:
        ...
        if self.cfg.save_mp4:
            ...
            # Force-integration cases: a vorticity movie (von Karman shedding)
            # and the drag / lift time history + Cl spectrum.
            if self.forces_calc is not None:
                self.viewer.animate("vort", f"{self.cfg.name}_vorticity.mp4",
                                    cmap="RdBu_r", title="Vorticity")
                self._plot_force_history()
        # history CSV
        if self.history:
            import csv
            path = os.path.join(self.cfg.output_dir, "history.csv")
    ...
    def _header(self) -> str:
        # SI compliance of the case file is checked and reported here
        # (units.validate_config) so non-SI / undocumented parameters surface
        # before the run; all banner quantities carry their coherent-SI unit.
        try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Hardware report at startup (GPU detected -> GPU path; else CPU fallback).
