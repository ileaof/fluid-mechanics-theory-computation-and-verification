...
class MomentumSolver:
    ...
    def predict(self, u, v, w, nu, dt, src_u, src_v, src_w):
        ...
        # Direct forcing: enforce the immersed-boundary no-slip on the
        # predictor so the pressure Poisson sees the right divergence source at
        # the body.  With the staircase this is a zero-clamp inside the solid;
        # with ``immersed_method:"ibm"`` the ghost cells are mirror-point
        # forced (no-slip at the true wall) and the deep interior is zeroed.
        if self.bc.has_solid:
            self.bc.apply_immersion(us)
            self.bc.apply_immersion(vs)
            if ws is not None:
                self.bc.apply_immersion(ws)
        return us, vs, ws